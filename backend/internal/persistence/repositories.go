...
func (r *pgConversationRepository) ListItems(ctx context.Context, participantID uuid.UUID, kind ParticipantType, convType *ConversationType) ([]ConversationListItem, error) {
	query := `
		SELECT conversation_id, reference_id, reference_type, item_name, last_message_at FROM (
			SELECT
	...
	for rows.Next() {
		...
		var refType string
		if err := rows.Scan(&item.ID, &item.ReferenceID, &refType, &item.Name, &item.LastMessageAt); err != nil {
